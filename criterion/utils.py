...
def compute_eer(label, pred):
    ...
    # === NEW: Handle edge case - all same label ===
    ...
    # === NEW: Handle edge case - all NaN ===
    diff = np.absolute(fnr - fpr)
    if np.all(np.isnan(diff)):
        return np.nan
    
    # the threshold of fnr == fpr
    eer_threshold = threshold[np.nanargmin(diff)]

    # theoretically eer from fpr and eer from fnr should be identical but they can be slightly differ in reality
    eer_1 = fpr[np.nanargmin(diff)]
    eer_2 = fnr[np.nanargmin(diff)]
...
class eer(Metric):
    ...
    def update(self, y_true, y_pred):
        eer_value = compute_eer(y_true, y_pred)
    
        # === NEW: Skip if computation failed ===
        if np.isnan(eer_value):
            return
    
        self.score += torch.tensor(eer_value, dtype=self.score.dtype)
