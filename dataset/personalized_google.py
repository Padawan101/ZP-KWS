"""
Personalized Google Speech Commands Dataset

Extends GoogleCommandsDataset with speaker pairing for P-KWS evaluation.

Speaker ID is extracted from filename:
    {speaker_hash}_{nohash}_{id}.wav → speaker_id = speaker_hash
    Example: 0a7c2a8d_nohash_0.wav → "0a7c2a8d"
"""

import os
import numpy as np
import torch
from .google import GoogleCommandsDataset


class PersonalizedGoogleCommandsDataset(GoogleCommandsDataset):
    """
    Google Speech Commands with speaker pairing support for P-KWS evaluation.
    
    Args:
        batch_size: Batch size
        personalized: Enable speaker pairing
        speaker_ratio: Ratio of same-speaker pairs (default: 0.5)
        **kwargs: Arguments passed to GoogleCommandsDataset
    
    Additional Returns (when personalized=True):
        enrollment_audio: Enrollment audio tensor
        speaker_label: 1 if same speaker, 0 if different
        final_label: 1 only if keyword AND speaker match
    ...
    def _extract_speaker_id(self, wav_path):
        """
        Extract speaker ID from GSC filename.
        
        Format: {speaker_hash}_{nohash}_{id}.wav
        Example: 0a7c2a8d_nohash_0.wav → "0a7c2a8d"
        """
    ...
    def _prepare_speaker_info(self):
        ...
        print(">> [Personalized GSC] Extracting speaker IDs...")
        
        # Extract speaker IDs for all samples
    ...
    def _select_enrollment_audio(self, target_speaker, exclude_idx=None):
        ...
        if len(candidates) == 0:
            # Fallback: use same utterance if no others available
    ...
    def __getitem__(self, idx):
        ...
        # Calculate final label (positive only if BOTH match)
