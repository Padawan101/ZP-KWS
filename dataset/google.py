...
class GoogleCommandsDataset(torch.utils.data.Dataset):
    def __init__(self, 
                 batch_size,
                 fs = 16000,
                 wav_dir='/padawan/google_speech_commands',
                 gemb_dir=None,
    ...
    def __getitem__(self, idx):
        ...
        if self.gemb_dir is not None:
            try:
                ...
                parts = self.wav_list[i].split(os.sep)
                if len(parts) >= 2:
            ...
            candidates = [
                os.path.join(self.gemb_dir, 'google_speech_commands', npy_rel_path),  # Actual directory name
                os.path.join(self.gemb_dir, 'gsc', npy_rel_path),  # Legacy name
